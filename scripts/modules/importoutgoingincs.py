		...
		with open(csvfile) as f:
			for line in f:
				...
				post = {"organization":linesplit[0].replace('"','').replace('\\',''),
						"from":linesplit[3].replace('"','').replace('\\',''),
						"date":linesplit[4].replace('"','').replace('\\',''),
						"uknown1":linesplit[5].replace('"','').replace('\\',''),
						"uknown2": linesplit[6].replace('"','').replace('\\',''),
						"assignedto": linesplit[7].replace('"','').replace('\\',''),
						"INCnr":linesplit[8].replace('"','').replace('\\',''),
						"nr":str(linesplit[9].replace('"','').replace('\\','')),
						"creationsource":linesplit[10].replace('"','').replace('\\',''),
						"changeevent":linesplit[12].replace('"','').replace('\\',''),
						"category":linesplit[14].replace('"','').replace('\\',''),
						"categorytype":linesplit[15].replace('"','').replace('\\','')}
